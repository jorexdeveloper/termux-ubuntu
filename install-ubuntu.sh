...
print_logo() {
	local spaces=""
	for ((i = $(((($(stty size | cut -d ' ' -f2) - 44) / 2))); i > 0; i--)); do
		spaces+=" "
	done
	clear
	msg -a "${spaces}┌──────────────────────────────────────────┐"
	msg -a "${spaces}│${G} _   _  ____   _   _  _   _  _____  _   _ ${C}│"
	msg -a "${spaces}│${G}| | | || __ ) | | | || \ | ||_   _|| | | |${C}│"
	msg -a "${spaces}│${G}| | | ||  _ \ | | | ||  \| |  | |  | | | |${C}│"
...
extract_rootfs_archive() {
	if [ -z "${KEEP_ROOTFS_DIRECTORY}" ]; then
		...
		set +e
		if proot --link2symlink tar --strip=0 --delay-directory-restore --warning=no-unknown-keyword --extract --gz --exclude="dev" --file="${ARCHIVE_NAME}" --directory="${ROOTFS_DIRECTORY}" --checkpoint=1 --checkpoint-action=ttyout="${I}${Y}    Extracted %{}T in %ds%*\r${N}${V}" &>>"${LOG_FILE}"; then
...
set_user_shell() {
	if [ -x "${ROOTFS_DIRECTORY}/usr/bin/chsh" ] && { if [ -z "${shell}" ]; then ask -n -- -t "Change the default login shell?"; fi; }; then
		...
		if [[ ${shells[*]} == *"${shell}"* ]] && [ -x "${ROOTFS_DIRECTORY}/usr/bin/${shell}" ] && distro_exec /usr/bin/chsh -s "/usr/bin/${shell}" root; then
